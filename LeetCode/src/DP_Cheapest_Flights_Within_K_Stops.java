public class DP_Cheapest_Flights_Within_K_Stops {
	public static void main(String[] args) {

		int[][] mat = { { 0, 12, 28 }, { 5, 6, 39 }, { 8, 6, 59 }, { 13, 15, 7 }, { 13, 12, 38 }, { 10, 12, 35 },
		...
		System.out.println(findCheapestPrice(17, mat, 13, 4, 13));
	}
	...
	public static int findCheapestPrice(int n, int[][] flights, int src, int dst, int k) {
		...
		for (int i = 0; i < flights.length; i++) {
			isPath[flights[i][0]][flights[i][1]] = true;
			map[flights[i][0]][flights[i][1]] = flights[i][2];
		}
		int t = help(src, map, isPath, dst, k);
	...
	public static int help(int i, int[][] map, boolean[][] isPath, int dest, int k) {
		if (i == dest) {
			return 0;
		}
		if (k < 0) {
		...
		for (int j = 0; j < map[0].length; j++) {
			if (isPath[i][j]) {
				int t = help(j, map, isPath, dest, k - 1);
				if (t != Integer.MAX_VALUE) {
					min = Integer.min(min, t + map[i][j]);
				}
			}
		}
		isVisited[i][k] = true;
